...
from sd_avg_se_pertot import open_dataset, calculate_mineral
from table2raster import save_nc
from scientific_colormaps import gm2_map as gm2
from scientific_colormaps import batlowK_map as batlowK
...
pforms = ['Inorganic P', 'Organic P', 'Available P',
          'Total P', 'Primary mineral P + Occluded P']

var = ['inorg_p', 'org_p', 'avail_p', 'total_p', 'mineral_p']

names_P = ("SMP", "OP", "AP","TP", "MOP")

var_acronym = dict(zip(var, names_P))
...
mask = ShapelyFeature(Reader("./inputDATA/mask.shp").geometries(),
                                 ccrs.PlateCarree())
...
def make_DI_mask(pfrac):
    assert pfrac in var
    try:
        di = np.load(f"AOA_{var_acronym[pfrac]}.npy")
        di_masked = np.ma.masked_array(di, di == -9999.0, fill_value=-9999.0)
    except:
        di = np.load(f"AOA_TP.npy")
        di_masked = np.ma.masked_array(di, di == -9999.0, fill_value=-9999.0)
        if pfrac == "mineral_p":
            return di_masked.mask
    return di_masked >= percentile_Treshold(di_masked.data)
...
def plot_Pmap(vname):
    ...
    if vname == 'total_p':
        frac_of_total = None
        img = open_dataset(vname, fun='mean')
        ...
        ds.close()
        mask_di = make_all_mask()
        nmodels = ""
        Amean = ""
        frac_of_total = np.ma.masked_array(frac_of_total, mask=mask_di)
        img = np.ma.masked_array(calculate_mineral(), mask=mask_di)
    else:
        perc_of_tot = Dataset(f"./{vname}_percent_of_tot.nc4")

        frac_of_total = np.flipud(perc_of_tot.variables[f"{vname}percent_of_total_p"][:])
        perc_of_tot.close()
        img = open_dataset(vname, fun='mean')

    if not vname == "mineral_p":
        SD = np.flipud(Dataset("./%s_STD.nc4" % vname).variables[vname + "_STD"])
        SE = np.flipud(Dataset("./%s_SE.nc4" % vname).variables[vname + "_SE"])
    ...
    ax.add_geometries(Reader("./inputDATA/mask.shp").geometries(),
                  ccrs.PlateCarree(),
    ...
    if frac_of_total is not None:
        ...
        ax1.add_geometries(Reader("./inputDATA/mask.shp").geometries(),
                  ccrs.PlateCarree(),
                  facecolor='whitesmoke', hatch='/////////', zorder=1, linewidth=0.2)
        ...
        if not vname == 'mineral_p':
            ...
            ax2.add_geometries(Reader("./inputDATA/mask.shp").geometries(),
                  ccrs.PlateCarree(),
                  facecolor='whitesmoke', hatch='/////////', zorder=1, linewidth=0.2)
